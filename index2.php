...
<html>
	<head>
		...
		<link rel=stylesheet type='text/css' href='http://tpatp.ru/style.css'>
		<meta HTTP-EQUIV='Content-Type' content='text/html; charset=windows-1251'>
		<script type='text/javascript' src='http://tpatp.ru/js/iepngfix_tilebg/iepngfix_tilebg.js'></script>
		<script type='text/javascript' src='http://tpatp.ru/js/lib.js'></script>
	</head>
										...
										<div class=content "; if( $r==4 ){ print"style='width: 720px;'"; } print">";
											...
											elseif( $r==4 )	// расписание движения
											{
												...
												if( isset( $_POST["find"] ) || isset( $_GET["type"] ) )
												{
													...
													for( $j=0;$j<=$ticket;$j++ )
													{
														...
														$query="select * from auto_path, auto_route where auto_route.id=auto_path.route and route $par1 '$route' and type $par4 '$type' and point_otp $par2 '$point_otp_' and point_prib $par3 '$point_prib_' order by nomer";
											...
											elseif( $r==3 )	// вопрос-ответ
											{
												...
												if( isset( $_POST["send"] ) )
												{
													...
													if( $_POST["name"]!="" && $_POST["message"]!="" && $_POST["name"]!=""  )
													{
														$_POST["name"]=simple_text( $_POST["name"] );
														$_POST["message"]=simple_text( $_POST["message"] );
														$_POST["email"]=simple_text( $_POST["email"] );
														mysql_query("insert into question values('', '".$_POST["message"]."', '', '".$_POST["name"]."', '".$_POST["email"]."' )");
														/*
														...
														mail("$admin_email4", "Обратная связь", "$message", $headers);
														*/
												...
												print"<form action=index.php?r=$r method=post>
													...
													<div class=left>&nbsp;</div>
													<div><input type=submit name=send value='Отправить'></div><br>
													<div class=left>&nbsp;</div>
													<div>$error</div>
												</form><br />";
											...
											{
												print"<link rel='stylesheet' href='http://tpatp.ru/js/lightbox/lightbox.css' type='text/css' media='screen' />
												<script src='http://tpatp.ru/js/lightbox/prototype.js' type='text/javascript'></script>
												<script src='http://tpatp.ru/js/lightbox/scriptaculous.js?load=effects,builder' type='text/javascript'></script>
												<script src='http://tpatp.ru/js/lightbox/lightbox.js' type='text/javascript'></script>";
											...
											elseif( $r==28 )	// бланк резюме
											{
												...
												print"<script src=http://tpatp.ru/js/calendar/calendar_ru.js></script>
												<script>
											...
											else	// остальные разделы
											{
												...
												if( $r==35 )	// технический осмотр
												{
													...
													if( isset( $_POST["send"] ) )
													{
														if( $_POST["number"]!="" && $_POST["seria"]!="" && $_POST["email"]!=""  )
														{
															$_POST["fio"]=simple_text( $_POST["fio"] );
															$_POST["number"]=simple_text( $_POST["number"] );
															$_POST["seria"]=simple_text( $_POST["seria"] );
															$_POST["email"]=simple_text( $_POST["email"] );
															$_POST["phone"]=simple_text( $_POST["phone"] );
															$_POST["date"]=simple_text( $_POST["date"] );
													...
													print"<br><form action=index.php?r=$r method=post>
														...
														<div class=left2>&nbsp;</div>
														<div><input type=submit name=send value='Отправить заявку'></div><br>
														<div class=left2>&nbsp;</div>
														<div>$error</div>
													</form>";
												}
												...
												if( $r==6 )	// заказ автобусов
												{
													...
													if( isset( $_POST["send"] ) )
													{
														if( $_POST["fio"]!="" && $_POST["phone"]!="" && $_POST["from"]!="" && $_POST["to"]!="" )
														{
															$_POST["time"]=simple_text( $_POST["time"] );
															$_POST["from"]=simple_text( $_POST["from"] );
															$_POST["to"]=simple_text( $_POST["to"] );
															$_POST["fio"]=simple_text( $_POST["fio"] );
															$_POST["phone"]=simple_text( $_POST["phone"] );
															$headers= "MIME-Version: 1.0\r\n";
															$headers .= "Content-type: text/html; charset=windows-1251\r\n";
												...
												{
													print"<br><a href=index.php?r=$r class=name "; if( $_GET["n"]!=0 ){ print"style='color: #b3b3b3; border-bottom: 1px dashed #b3b3b3;'"; } print">Реализуем</a> &nbsp; &nbsp; <a href=index.php?r=$r&n=1 class=name "; if( $_GET["n"]!=1 ){ print"style='color: #b3b3b3; border-bottom: 1px dashed #b3b3b3;'"; } print">Купим</a><br><br>";
													$view_page=20;
